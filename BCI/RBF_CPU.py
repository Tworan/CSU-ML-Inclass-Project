...
class Cluster:
    ...
    def fit(self, data):
        ...
        distance_matrix = np.array([
            np.sum(np.square(data - self.cluster_center[i]), axis=-1) for i in range(self.cluster_num)
        ])
        this_belongs = np.argmin(distance_matrix, axis=0)
        last_belongs = np.zeros_like(this_belongs)
        
        def if_two_array_equal(arr1, arr2, softmargin=0):
            return True if np.sum(arr1 != arr2) <= softmargin else False

        while not if_two_array_equal(this_belongs, last_belongs, 0.05 * len(data)):
            last_belongs = this_belongs.copy()
            
            self.cluster_center = np.array([np.mean(data[np.where(last_belongs == i)], axis=0) for i in range(self.cluster_num)])
            distance_matrix = np.array([
                np.sum(np.square(data - self.cluster_center[i]), axis=-1) for i in range(self.cluster_num)
            ])
            this_belongs = np.argmin(distance_matrix, axis=0)
    ...
    def initial_cluster_center(self, data):
        cluster_center = np.array(data[np.random.choice(len(data), 1)])
        k = 1
        vis = [0] * len(data)
        while k < self.cluster_num:
            distance_ = [np.sum(np.square(data - cluster_center[i]), axis=-1) for i in range(len(cluster_center))]
            distance = np.zeros_like(distance_[0])
            for dist in distance_:
                distance += dist 

            i = 0
            while True:
                i -= 1
                next_cluster_center_index = np.argsort(distance)[i]
                if vis[next_cluster_center_index] == 0:
                    vis[next_cluster_center_index] = 1
                    break 
            cluster_center = np.concatenate([cluster_center, data[next_cluster_center_index].reshape((1, ) + data[next_cluster_center_index].shape)], axis=0)
            k += 1
...
def grads_clip(grads, clipvalue=1.):
    grads[np.where(np.abs(grads) > clipvalue)] = clipvalue
    return grads
...
def cosine_annealing(epochs, cur_epoch, cycles, base_lr):
    ...
    cos_inner = (np.pi * (cur_epoch % epoch_per_cycle)) / epoch_per_cycle
    return base_lr / 2 * (1 + np.cos(cos_inner))
...
class RBF:
    ...
    def fit(self, X, Y, epochs=1000):
        ...
        var = cmax[:, 0] / np.sqrt(2. * self.hidden_nodes)
    ...
    def predict(self, X):
        ...
        self.active_values = np.array(active_values)
...
def GaussionInit(shape, num):
    return np.random.rand(*shape) * np.sqrt(1. / num)     
...
class activation:
    ...
    def forward(self, x):
        bias = (x - self.c)
        radius = np.sum(bias * bias, axis=1)
        active_value = np.exp(-1. / (2. * self.var * self.var + 1e-7) * radius)
        return active_value
